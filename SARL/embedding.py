...
model = resnet18(pretrained=True).float().to(device)#pretrained=True).float().to(device)#, non_blocking=True)


def get_embedding(image): #Used for converting an image into an embedding
    return model(torch.from_numpy(np.double(image))).to(device)#, non_blocking=True))
